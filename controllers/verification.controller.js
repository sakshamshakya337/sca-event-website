    ...
    if (req.files) {
      if (req.files.universityId) {
        try {
          ...
          console.warn('⚠️ Cloudinary upload failed for universityId in submitApplication, falling back to base64 Data URL:', err.message)
          const mimeType = req.files.universityId[0].mimetype || 'image/png'
          const base64Data = req.files.universityId[0].buffer.toString('base64')
          applicationData.universityIdUrl = `data:${mimeType};base64,${base64Data}`
          applicationData.universityIdPublicId = `fallback_id_${Date.now()}`
        }
      }
      ...
      if (req.files.profilePhoto) {
        try {
          ...
          console.warn('⚠️ Cloudinary upload failed for profilePhoto in submitApplication, falling back to base64 Data URL:', err.message)
          const mimeType = req.files.profilePhoto[0].mimetype || 'image/png'
          const base64Data = req.files.profilePhoto[0].buffer.toString('base64')
          applicationData.profilePhotoUrl = `data:${mimeType};base64,${base64Data}`
          applicationData.profilePhotoPublicId = `fallback_photo_${Date.now()}`
        }
      }
    }
