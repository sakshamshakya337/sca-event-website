  ...
  try {
    ...
    if (req.file) {
      ...
      try {
        ...
        console.warn('⚠️ Cloudinary upload failed, falling back to base64 Data URL:', cloudinaryError.message)
        // Fallback: Convert buffer to base64 data URI so the image is still saved and displayed correctly
        const mimeType = req.file.mimetype || 'image/png'
        const base64Data = req.file.buffer.toString('base64')
        user.profilePhotoUrl = `data:${mimeType};base64,${base64Data}`
        user.profilePhotoPublicId = `fallback_${user._id}`
      }
    }

    await user.save()

    if (oldProfilePhotoPublicId && oldProfilePhotoPublicId !== user.profilePhotoPublicId && !oldProfilePhotoPublicId.startsWith('fallback_')) {
      try {
        await cloudinary.uploader.destroy(oldProfilePhotoPublicId)
      } catch (err) {
        console.warn('⚠️ Failed to destroy old Cloudinary image:', err.message)
      }
    }
