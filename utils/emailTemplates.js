...
export const eventApprovedTemplate = ({ facultyName, eventTitle, slug }) => {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #ddd; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
      <div style="background-color: #28a745; padding: 30px; text-align: center;">
        <h2 style="color: white; margin: 0; font-size: 24px;">Event Approved! 🎉</h2>
        <p style="color: rgba(255,255,255,0.9); margin-top: 10px; font-size: 16px;">Your event proposal has been approved.</p>
      </div>
      <div style="padding: 40px 30px; background-color: #ffffff;">
        <p style="font-size: 16px; color: #333; margin-bottom: 20px;">Dear <strong>Prof. ${facultyName}</strong>,</p>
        <p style="font-size: 16px; color: #555; line-height: 1.5; margin-bottom: 25px;">
          We are pleased to inform you that your event proposal titled "<strong>${eventTitle}</strong>" has been reviewed and <strong>approved</strong> by the admin team.
        </p>
        <p style="font-size: 16px; color: #555; line-height: 1.5; margin-bottom: 30px;">
          The event is now active on the platform and visible to the public for registrations.
        </p>
        <div style="text-align: center;">
          <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/events/${slug}" 
             style="display: inline-block; background-color: #28a745; color: white; padding: 14px 35px; 
                    text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px; transition: background-color 0.3s;">
            View Event Details
          </a>
...
export const eventRejectedTemplate = ({ facultyName, eventTitle, reason }) => {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #ddd; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
      <div style="background-color: #d93025; padding: 30px; text-align: center;">
        <h2 style="color: white; margin: 0; font-size: 24px;">Event Proposal Update</h2>
        <p style="color: rgba(255,255,255,0.9); margin-top: 10px; font-size: 16px;">Action required on your event proposal.</p>
      </div>
      <div style="padding: 40px 30px; background-color: #ffffff;">
        <p style="font-size: 16px; color: #333; margin-bottom: 20px;">Dear <strong>Prof. ${facultyName}</strong>,</p>
        <p style="font-size: 16px; color: #555; line-height: 1.5; margin-bottom: 25px;">
          We have reviewed your event proposal titled "<strong>${eventTitle}</strong>". Unfortunately, we are unable to approve it at this time.
        </p>
        ...
        <p style="font-size: 15px; color: #666; line-height: 1.5; margin-top: 25px;">
          You can edit the event details based on the feedback and submit it again for approval.
        </p>
      </div>
      <div style="background-color: #f8f9fa; padding: 20px; text-align: center; font-size: 13px; color: #888; border-top: 1px solid #eee;">
        <p style="margin-bottom: 5px;">This is an automated message. Please do not reply.</p>
