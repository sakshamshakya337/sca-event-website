...
import { MdSearch, MdStar, MdStarBorder, MdLocationOn, MdCalendarToday } from 'react-icons/md'
          ...
          <tbody className="divide-y divide-outline-variant/30">
            {filteredEvents.map(event => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-body-sm font-semibold border ${statusStyles[normalizeEventStatus(event.status)] || statusStyles.pending}`}>
                    {normalizeEventStatus(event.status) === 'approved' && <span className="w-1.5 h-1.5 rounded-full bg-green-500" />}
                    {normalizeEventStatus(event.status) === 'pending' && <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />}
                    {normalizeEventStatus(event.status) === 'rejected' && <span className="w-1.5 h-1.5 rounded-full bg-red-500" />}
